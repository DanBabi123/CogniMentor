...
def resend_otp():
    ...
    if OTPService.send_otp(user_id):
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        flash('An OTP has been sent to your email. Please check your inbox.', 'modal-info')
        return redirect(url_for('auth.verify'))
...
def verify():
    ...
    if form.validate_on_submit():
        ...
        if success:
            ...
            return render_template('auth/verify.html', form=form, verification_success=True)
        else:
            flash(message, 'error')
