from models import QuizAttempt, Topic, LearningProgress
    ...
    def get_recommendation(self, user_id):
        ...
        if weak_attempt:
            ...
            return {
                ...
                "recommended_topic": topic.title, #type: ignore
                "difficulty_level": "Review",
                "study_time": "20 mins",
                "reason": f"You scored {weak_attempt.score}% on {topic.title}. Let's turn that into a win!",#type: ignore
                "link": topic.id #type: ignore
            }
        ...
        if last_success:
            ...
            next_topic = Topic.query.filter(Topic.id > current_topic.id).order_by(Topic.id.asc()).first()#type: ignore
            ...
            if next_topic:
                 return {
                    ...
                    "study_time": "45 mins",
                    "reason": f"Great job on {current_topic.title}! precise next step: {next_topic.title}.",#type: ignore
                    "link": next_topic.id
                }
